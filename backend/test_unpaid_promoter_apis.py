...
Run tests with: python manage.py test test_unpaid_promoter_apis.py
"""
...
from promoter.models import Promoter, PromotedProduct, ProductVariant, PremiumSettings
...
class UnpaidPromoterDashboardAPITestCase(APITestCase):
    ...
    def test_unpaid_dashboard_access_denied_for_unapproved_promoter(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('Your promoter account is not yet approved', response.json()['detail'])
...
if __name__ == '__main__':
    ...
    # Run tests
    execute_from_command_line(['test', 'test_unpaid_promoter_apis.py'])
